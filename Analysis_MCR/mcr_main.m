...
%      function named CALLBACK in MCR_MAIN.M with the given input
%      arguments.
%
...
% Last Modified by GUIDE v2.5 09-Feb-2025 22:50:38
...
% initializations
% clear variable mcr_als from previous session
...
%read variables from base workspace; it returns a structure lsvar with
%variables (name, size, byte, class)
if valorDT==0
    ...
    for i=1:aa,
        ...
        if csize==6,
            if lsvar(i).class=='double', %only double variables 
                lsb=[lsvar(i).name];
                lsv(j)={lsb};
                j=j+1;
            end;
...
if evalin('base','exist(''mcr_als'')')==1
    matdad=evalin('base','mcr_als.data');
end
num_pix=size(matdad, 1);
if num_pix>3000 % evaluate the number of rows, since they represent pixels
    disp('One dimension exceeds 3000: unable to plot all the dataset!');
    % only 3000 central pixels are considered for plotting
    matdad_small=matdad(floor(num_pix/2)-1400:floor(num_pix/2)+1400, :); 
    assignin("base", 'matdad_small', matdad_small); 
end
plot_dades;
...
%Manual estimation function: double variables from workspace are shown.
manEst;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Filename, Filepath] = uigetfile('*.mat', 'Select a file', "C:\");
assignin('base', 'Filename', Filename);
assignin('base', 'Filepath', Filepath);

% we load the chosen .mat file in the workspace
...
if size(Hyperspectrum_cube, 3)==2.*length(f) %if the spectral hypecube is complex the real and imag are stacked one over the other
    ...
    Hyperspectrum_cube_complex=Hyperspectrum_cube_real+1i.*Hyperspectrum_cube_imag;
    Hyperspectrum_cube=abs(Hyperspectrum_cube_complex); %I must work with the absolute values
    clear Hyperspectrum_cube_real Hyperspectrum_cube_imag
end

Hyperspectrum_cube=abs(Hyperspectrum_cube); %if hypercube is complex but saved in double
rows=size(Hyperspectrum_cube, 1); 
columns=size(Hyperspectrum_cube, 2);
depth=size(Hyperspectrum_cube, 3);
matdad=double(reshape(Hyperspectrum_cube, [rows*columns, depth])); %double type is necessary for the other operations
assignin('base', 'matdad', matdad);

evalin('base','mcr_als.OriginalData=matdad;');
evalin('base','mcr_als.data=matdad;');
evalin('base','mcr_als.aux.data_name=strcat(Filepath, Filename);');
disp('Dataset imported correctly!') %comment for dubugging- can be removed!
...
% activate: weight text
set(handles.text_weight,'string','No');
% show imported dataset path
set(handles.ImporteddatasetLabel, 'string', strcat(Filepath, Filename));
