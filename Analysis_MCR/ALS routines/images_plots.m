...
%save abundances maps in base workspace as cell variable
mdis=evalin('base','mcr_als.aux.mdis;');
...
%save abundances map 
if ((evalin('base','exist(''Filepath'')'))==1) %if dataset was imported, save in the same directory
    Filepath=evalin('base', 'Filepath'); 
    Filename='ConcMaps'; 
else %ask the user to choose the directory for saving
    [Filename, Filepath] = uiputfile('*.mat', 'Save abundances maps as matrices');
end
    file_tot=[Filepath,Filename];

for idx_species=1: size(mdis, 2)
    for idx_n_image=1: size(mdis,1)
        var2save=cell2mat(mdis(idx_n_image, idx_species)); 
        var2save=var2save/max(max(var2save)); %normalize each map for proper plotting 
        %.mat file
        save(strcat(file_tot,'_im','_', num2str(idx_n_image),...
            '_','spec','_', num2str(idx_species)), 'var2save');
        %.jpg file
        imwrite(var2save, strcat(file_tot,...
            '_im', num2str(idx_n_image),'_','spec','_', num2str(idx_species), '.tiff'));
    end
end
